...
def has_responded(account_name, user_id):
    ...
    if not os.path.exists(file_name):
        print(f"{file_name} does not exist. Returning False.")
        return False
    ...
    try:
        with open(file_name, 'r', encoding='utf-8') as f:
            ...
            if os.stat(file_name).st_size == 0:
                print(f"{file_name} is empty. Returning False.")
                return False
            
            responded_users = json.load(f)
    except json.JSONDecodeError:
        # Handle the case where the JSON is invalid/corrupt
        print(f"Failed to parse JSON in {file_name}. Returning False.")
        return False

    # Check if the user_id exists in the responded_users list/dict
